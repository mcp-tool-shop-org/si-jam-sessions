...
// ---------------------------------------------------------------------------------------
// The exemplars: two arrangements of Battle Hymn of the Republic in `scores/`, each
// generated by a model on Ollama Cloud from the project's transcription of the 1862 Ditson
// edition, and receipted from the same evidence as the fixture above.

const GLM_JSON: &[u8] = include_bytes!("../../../scores/battle-hymn-glm-5.3/receipt.json");
const GLM_LY: &[u8] = include_bytes!("../../../scores/battle-hymn-glm-5.3/battle-hymn-glm-5.3.ly");
const GLM_MID: &[u8] =
    include_bytes!("../../../scores/battle-hymn-glm-5.3/battle-hymn-glm-5.3.mid");
const KIMI_JSON: &[u8] = include_bytes!("../../../scores/battle-hymn-kimi-k3/receipt.json");
const KIMI_LY: &[u8] = include_bytes!("../../../scores/battle-hymn-kimi-k3/battle-hymn-kimi-k3.ly");
const KIMI_MID: &[u8] =
...
/// files as the predicate is handed them.
struct Exemplar {
    model: &'static str,
    json: &'static [u8],
    digest: &'static str,
    supplied: [Supplied<'static>; 2],
}

fn exemplars() -> [Exemplar; 2] {
    [
        Exemplar {
            model: "glm-5.3",
            json: GLM_JSON,
            ...
            supplied: [
                Supplied {
                    name: "battle-hymn-glm-5.3.ly",
                    bytes: GLM_LY,
                },
                Supplied {
                    name: "battle-hymn-glm-5.3.mid",
                    bytes: GLM_MID,
                },
            ],
        },
        Exemplar {
            ...
            supplied: [
                Supplied {
                    name: "battle-hymn-kimi-k3.ly",
                    bytes: KIMI_LY,
                },
                Supplied {
                    name: "battle-hymn-kimi-k3.mid",
                    bytes: KIMI_MID,
                },
            ],
        },
...
/// Each exemplar is admitted under predicate version 3 as this project's own engraving,
/// and its receipt says what is true of it: the work, the tune and the edition exactly as
/// the fixture records them (and the fixture is checked against the evidence manifest
/// above); the arrangement by this project, generated by the named model; its two files,
/// the LilyPond file stating CC0 1.0 and the MIDI file stating nothing; and notes that name
...
fn the_battle_hymn_exemplars_are_admitted_as_own_engravings() {
    ...
    for x in exemplars() {
        ...
        let id = format!("battle-hymn-{}", x.model);
        let admitted = admit(&r, &x.supplied).expect("admitted");
        assert_eq!(admitted.tier, Tier::OwnEngraving, "{id}");
        assert_eq!(admitted.receipt_digest, r.digest(), "{id}");
        assert_eq!(hex(&r.digest()), x.digest, "{id}");

        assert_eq!(r.score_id, id);
        assert_eq!(r.title, fixture.title);
        assert_eq!(r.fetched_on, fixture.fetched_on);
        assert_eq!(r.composition, fixture.composition, "{id}");
        assert_eq!(r.source_edition, fixture.source_edition, "{id}");
        assert_eq!(r.evidence, fixture.evidence, "{id}");
        assert_eq!(
            r.arrangement,
            Arrangement::ThisProject(ThisProject {
                engraver: format!(
                    "si-jam-sessions (arrangement generated by {} on Ollama Cloud)",
                    x.model
                ),
            })
        );
        ...
        assert_eq!(names, [format!("{id}.ly"), format!("{id}.mid")]);
        assert_eq!(r.files[0].media, Media::Lilypond);
        assert_eq!(r.files[1].media, Media::Smf);
        for (f, s) in r.files.iter().zip(&x.supplied) {
            assert_eq!(f.bytes, s.bytes.len() as u64, "{}", f.name);
            assert_eq!(f.sha256, sha256(s.bytes), "{}", f.name);
            assert_eq!(
                f.url,
                format!(
                    "https://github.com/mcp-tool-shop-org/si-jam-sessions/blob/main/scores/{id}/{}",
                    f.name
                )
            );
            assert_eq!(f.last_modified, None, "{}", f.name);
        }
        assert_eq!(
            r.files[0].in_file_licence,
            vec![Statement {
        ...
        assert_eq!(r.files[1].in_file_licence, vec![], "{id}");

        // The notes: how the arrangement was made, then the fixture's notes on the work,
        // the tune and the edition, word for word.
        let notes = r.notes.join("\n");
        for needle in [
            format!("generated for this project by the model {}", x.model),
            String::from("on Ollama Cloud"),
            String::from("research/battle-hymn/battle-hymn-ditson-1862.abc"),
            String::from("def065f78f07b0ced5473bc74c13173a28f5cd25ed6368bb9a2bfe6c3c51e161"),
            String::from("LilyPond 2.24.4"),
            String::from("under CC0 1.0"),
        ] {
            assert!(notes.contains(&needle), "{id}: {needle}");
        }
        let language_line = notes.contains("\\language \"english\"");
        assert_eq!(language_line, x.model == "kimi-k3", "{id}");
        for n in &fixture.notes[1..fixture.notes.len() - 1] {
            assert!(r.notes.contains(n), "{id} lacks the fixture's note: {n}");
        }
        for e in &r.evidence {
            assert!(
                r.notes.iter().any(|n| n.contains(e.id.as_str()))
                    || r.composition.evidence.contains(&e.id)
                    || r.source_edition.evidence.contains(&e.id),
                "{id}: {} is cited nowhere",
                e.id
            );
        ...
        let bytes = r.to_canonical();
        assert_eq!(Receipt::from_canonical(&bytes).as_ref(), Ok(&r));
        let text = core::str::from_utf8(x.json).unwrap();
        for needle in [":\\", "/home/", "\\Users\\", "/Users/", "file:"] {
            assert!(!text.contains(needle), "{id}: {needle}");
        }
        assert_eq!(text.matches(":/").count(), text.matches("://").count());
    }
}

/// The receipts pin their files: a changed byte, a file swapped for the other exemplar's,
/// or a receipt that claims the MIDI file states CC0 1.0 is refused.
#[test]
fn an_exemplar_is_refused_for_a_changed_or_swapped_file() {
    let [glm, kimi] = exemplars();
    let r = Receipt::from_json(glm.json).unwrap();
    let mut ly = GLM_LY.to_vec();
    let last = ly.len() - 1;
    ly[last] ^= 1;
    let changed = [
        Supplied {
    ...
    assert_eq!(
        ...
        Err(Refusal::HashMismatch {
            name: named("battle-hymn-glm-5.3.ly")
        })
    );
    let swapped = [
        Supplied {
            name: "battle-hymn-glm-5.3.mid",
            bytes: kimi.supplied[1].bytes,
        },
        glm.supplied[0],
    ];
    assert_eq!(
        ...
        Err(Refusal::SizeMismatch {
            name: named("battle-hymn-glm-5.3.mid")
        })
    );
    let mut claims = Receipt::from_json(kimi.json).unwrap();
    claims.files[1].in_file_licence = vec![Statement {
        field: StatementField::SmfCopyright,
        text: named(OWN_ENGRAVING_LICENCE),
    }];
    assert_eq!(
        admit(&claims, &kimi.supplied),
        Err(Refusal::InFileMisrecorded {
            name: named("battle-hymn-kimi-k3.mid")
        })
    );
}
